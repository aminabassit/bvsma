...
// ./verifierParallel 192 zkPoKMultiANDPlain
// ./verifierParallel 224 zkPoKMultiANDPlain
// ./verifierParallel 256 zkPoKMultiANDPlain
...
int main(int argc, char* argv[]){
    ...
    try
    {
        ...
        if ( zkProof == "zkPoKBasic") {
            ...
            start = std::chrono::system_clock::now();
            auto result1 = zkPoKMultiANDPlainVerifier_parallel(verifier, configFile, curve, proverPubKey, vectCipher);
            print_elapsed_ms(start, zkProof);
            cout << "Parallel Verifier verifies the result: " << (result1 ? "Success" : "Failure") << endl;
            ...
            start = std::chrono::system_clock::now();
            auto result1 = niZKMultiANDDecZeroVerifier_parallel(verifier, configFile, curve, proverPubKey, cipherDecZeroVect);
            print_elapsed_ms(start, zkProof);
            cout << "Parallel Verifier verifies the result: " << (result1 ? "Success" : "Failure") << endl;
            ...
            start = std::chrono::system_clock::now();
            auto result1 = niZKMultiANDBlindedVerifier_parallel(verifier, configFile, curve, proverPubKey, vectCipher3, vectBlindedCipher);
            print_elapsed_ms(start, zkProof);
            cout << "Parallel Verifier verifies the result: " << (result1 ? "Success" : "Failure") << endl;


            start = std::chrono::system_clock::now();
            auto result = niZKMultiANDBlindedVerifier(verifier, dlog, elgamal, vectCipher3, vectBlindedCipher);
            print_elapsed_ms(start, zkProof);
            cout << "Verifier verifies the result: " << (result ? "Success" : "Failure") << endl;
        }
            ...
            start = std::chrono::system_clock::now();
            auto result1 = niZKMultiANDPartialDecVerifier_parallel(verifier, configFile, curve, provThreshPubKey, vectCipherForPartial, vectPartialCipher);
            print_elapsed_ms(start, zkProof);
            cout << "Parallel Verifier verifies the result: " << (result1 ? "Success" : "Failure") << endl;
